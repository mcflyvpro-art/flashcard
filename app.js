/* Cartes — flashcards PWA, zero dépendance, tout en local */
const KEY = 'cartes.v1';
...
function importPayload(p) {
  if (!p) return null;
  const packs = Array.isArray(p) ? p : [p];
  let last = null;
...
/* ---------- decks livrés avec l'app (dossier /decks) ---------- */
...
function home() {
  ...
  $.innerHTML = `
    ...
    ${db.decks.length ? `<div class="grid">${cards}</div>`
      : `<div class="empty">${svg(I.stack)}</div>`}
...
function startStudy(id) {
  ...
  const q = d.cards.map(c => c.id);
  for (let i = q.length - 1; i > 0; i--) { const j = (Math.random() * (i + 1)) | 0;[q[i], q[j]] = [q[j], q[i]]; }
  study = { id, queue: q, i: 0, again: [], flip: false, ok: 0, total: q.length };
...
function importView() {
  ...
  $.innerHTML = `
    ...
    <div class="sheet">
      ...
      <textarea id="tx" placeholder="chat = cat&#10;chien = dog" autocapitalize="off" spellcheck="false"></textarea>
  ...
  const up = () => { const n = parseText(tx.value).length; cnt.textContent = n; ok.disabled = !n; };
  tx.addEventListener('input', up);
  tx.focus();
  ok.onclick = () => {
    ...
    else { const nm = document.getElementById('nm').value; const d = addDeck(nm, cards); go('deck', d.id); }
    toast('+' + cards.length);
  };
}
...
$.addEventListener('click', e => {
  const b = e.target.closest('[data-act],[data-go],[data-rm],[data-a]'); if (!b) return;
...
window.addEventListener('hashchange', () => { if (consumeHash()) return; });
syncRepo();
